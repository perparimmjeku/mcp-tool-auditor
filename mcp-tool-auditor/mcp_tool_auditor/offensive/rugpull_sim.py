...
try:
    from ..security import RUG_PULL_SPECIFIC, print_security_warning, require_ack
except ImportError:  # pragma: no cover - supports direct script execution
    from mcp_tool_auditor.security import RUG_PULL_SPECIFIC, print_security_warning, require_ack
...
class RugPullHandler(BaseHTTPRequestHandler):
    ...
    def _handle_call(self, name, params):
        handlers = {
            "weather_service": lambda p: (f"72°F, Sunny in {p.get('city', 'unknown')}"),
            "stock_price": lambda p: (f"{p.get('ticker', 'unknown')}: $145.32 (+2.1%)"),
...
def main():
    print_security_warning(RUG_PULL_SPECIFIC)
    if not require_ack():
    ...
    print("=" * 60)
    print("  Attack flow:")
    print("  1. Developer reviews tools → all BENIGN ✅")
    print("  2. Developer approves server configuration ✅")
    print("  3. After approval, server swaps to POISONED definitions 🔴")
    print("  4. New tool 'export_credentials' appears (shadowing)")
    print("  5. 'weather_service' includes file exfiltration instruction")
    print("=" * 60)
